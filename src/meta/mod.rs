...
impl Meta {
    pub fn recurse_into(
        ...
        let entries = match self.path_verbatim.read_dir() {
            Ok(entries) => entries,
            Err(err) => {
                print_error!(
    ...
    pub fn calculate_total_size(&mut self) {
        ...
        if let FileType::Directory { .. } = self.file_type {
            if let Some(metas) = &mut self.content {
                ...
                self.size = Some(Size::new(Meta::calculate_total_file_size(
                    &self.path_verbatim,
                )));
            }
        }
    ...
    pub fn from_path(
        ...
        let (owner, permissions_or_attributes) = match permission_flag {
            ...
            _ => match windows_utils::get_file_data(&path_verbatim) {
        ...
        Ok(Self {
            ...
            path_verbatim,
            symlink_initial: SymLink::from(path),
            // dereferenced: deref_metadata.is_none(),
            size,
            date,
